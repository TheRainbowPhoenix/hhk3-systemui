...
public:
  MyWindow(PegRect &rect) : PegDecoratedWindow(rect) {
    ...
    infoBox = new PegTextBox(textRect, Id_InfoText, FF_RECESSED | EF_WRAP | TJ_LEFT,
  ...
  SIGNED Message(const PegMessage &mesg) override {
    ...
    // This is fired when the user navigates the list (up/down) or clicks an item.
    case PEG_SIGNAL(Id_List, PSF_LIST_SELECT):
      {
        // Get the ID of the currently selected item
        // Since we don't have GetSelectedId mapped, we can get the object and check its ID
        PegThing_Thin* selected = vertList->GetSelected();
        if (selected) {
           WORD id = selected->Id();
           
           if (id == Id_Btn_1) infoBox->DataSet("Option 1 Selected");
           else if (id == Id_Btn_2) infoBox->DataSet("Option 2 Selected");
           else if (id == Id_Btn_3) infoBox->DataSet("Option 3 Selected");
           else if (id == Id_Btn_4) infoBox->DataSet("Option 4 Selected");
    ...
    // But typically in a list, you handle the selection or an "Enter" key press on the list.
